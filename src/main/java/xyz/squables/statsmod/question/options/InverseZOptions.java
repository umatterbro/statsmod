...
import xyz.squables.statsmod.question.options.variables.types.OperationType;
import xyz.squables.statsmod.question.options.variables.types.VariableType;
...
public class InverseTOptions extends QuestionOptions {
    ...
    public <T extends Number> Answers<T> generateAnswers(QuestionType qt, Class<T> tClass) {
        if(qt.type == VariableType.DOUBLE) {
            this.area.setValue(Util.roundToN(this.area.getValue(), 3));
